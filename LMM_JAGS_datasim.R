...
#data needs to be simulated to then compare to the model
...
#We directly normalize covariate length to avoid trouble with WinBUGS. (Not sure what this means.) You're supposed to normalize all the covariates that go in
...
#when in a linear equation, you have a vector of [y] and a vector of covariates [ alpha, beta]
#multiply the design matrix by the covariates to get the vector of y
#this underlies every linear model
...
#lots of 0 because so many data points (560), just assign 1 to whatever you want the data values to correpond with.

#so this matrix is[ y1:y560] = [pop1:pop56] [mu1:mu56] + [e1:e560]


# We assemble the measurements yi as before.
lin.pred <- Xmat[,] %*% all.effects # Value of lin.predictor, specifies which effects to include for each y
...
library("lattice")
xyplot(mass ~ length | pop) #relationship between length and mass for every population from the data that we simulated
...
#We construct the relationship between vegetation density and lizard abundance
